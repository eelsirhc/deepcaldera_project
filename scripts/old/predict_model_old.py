...
def long_lat_rad_km_from_pix_geo(lat_0, lon_0, box_size, features, dim=256):
...
#    def fog2(lat_0, lon_0, box_size, src, src_data, dim=256):
    ...
    res=[]
    for irow, row in features.iterrows():
        ypos, xpos, dpos = row["x"], row["y"], row["Diameter (pix)"]
        c = reproject_coords(orthographic, latlong, [tr.xy(xpos, ypos)])[0]
        di = 1e-3*abs(tr.xy(xpos, ypos+dpos//2)[0]-tr.xy(xpos, ypos-dpos//2)[0])
...
def update_unique_craters(
    ...
    start = np.clip(start, 0, P["input_DEM"].shape[0] - 1)
    stop = np.clip(stop, 1, P["input_DEM"].shape[0])
    craters_h5 = pd.HDFStore(CP["dir_craters"], "a")
...
#        print(img,i)
        ...
        coords = craters_h5[img][["x","y","Diameter (pix)"]].values * np.array([1, 1, .5])[None, :]
        # convert, add to master dist
        if len(coords) > 0:
            #print(P['box_size'][i], P['central_lat_lon'][i], coords)
...
def make_prediction(llt2, rt, index, prefix, start, stop, matches, model, dataset):
    ...
    update_unique_craters(CP, index)
